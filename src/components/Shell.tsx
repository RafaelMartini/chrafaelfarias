...
const studentNav = [
  ...
  { label: "Anamnese", to: "/aluno/anamnese" },
  { label: "Comparação", to: "/aluno/comparacao" },
...
export function Shell({
  ...
  return (
    <div className="min-h-screen bg-transparent">
            ...
            {nav.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={
                    active
                      ? "rounded-full bg-primary px-4 py-2 text-primary-foreground shadow-lg"
                      : "rounded-full px-4 py-2 hover:bg-secondary hover:text-foreground transition-colors"
                  }
                >
                  {item.label}
                </Link>
      ...
      {/* Navegação no mobile — as abas do topo ficam escondidas em telas pequenas. */}
      <div className="md:hidden mx-auto mt-2 w-[calc(100%-1.5rem)] max-w-7xl overflow-x-auto">
        <div className="flex w-max gap-2 px-1 py-1 text-[11px] font-semibold uppercase tracking-widest text-muted-foreground">
          {nav.map((item) => {
            const active = pathname === item.to;
            return (
              <Link
                ...
                className={
                  active
                    ? "shrink-0 rounded-full bg-primary px-4 py-2 text-primary-foreground shadow-lg"
                    : "shrink-0 rounded-full border border-border px-4 py-2 hover:bg-secondary hover:text-foreground transition-colors"
                }
              >
                {item.label}
              </Link>
      ...
      <footer className="mx-auto mb-6 mt-20 max-w-7xl rounded-3xl border border-border bg-card/60 px-6 py-10 backdrop-blur-xl">
        <div className="mx-auto flex flex-col items-center justify-between gap-8 md:flex-row">
          ...
          <div className="flex gap-8">
            <a
              href="#"
              className="text-xs font-extrabold uppercase tracking-widest hover:text-primary"
            >
              Privacidade
            </a>
            <a
...
export function MetricCard({
  ...
  return (
    <div
      className={`rounded-3xl border border-border bg-card/80 p-6 shadow-2xl backdrop-blur-xl ${highlight ? "bg-primary/10 border-primary/40" : "hover:border-primary/40"} transition-all hover:-translate-y-1`}
    >
      <p
        className={`text-xs font-mono uppercase mb-4 ${highlight ? "text-primary" : "text-muted-foreground"}`}
      >
        {label}
      </p>
      ...
      {hint && (
        <p className="text-[10px] mt-4 uppercase text-muted-foreground tracking-tight">{hint}</p>
      )}
    </div>
  );
}
